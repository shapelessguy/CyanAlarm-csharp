...
namespace CyanAlarm
{
    public class FirebaseClass
    {
        ...
        public static void UploadOnStorage(string file)
        {
            filePath = file;
            System.Threading.Thread register = new Thread(Register);
            register.Start();
        }
        public static void DownloadIP()
        {
            System.Threading.Thread ipget = new Thread(GetIP);
            ipget.Start();
        }
        private static async void Register()
        {
            if (token == null || (int)((DateTime.Now - token.Created).TotalSeconds) > 10) { await FirebaseClass.CheckFirebaseCred("shapelessguy@hotmail.it", "180393acer"); FirebaseClass.FireBaseLogIn(); }
            if (token == null) { Console.WriteLine("LogIn richiesto.."); error = true; }
            await UploadFile_onStorage(filePath, "user/" + token.User.LocalId + "/Records/"+ Path.GetFileName(filePath));
        }
        private static async void GetIP()
        {
            if (token == null || (int)((DateTime.Now - token.Created).TotalSeconds) > 10) { await FirebaseClass.CheckFirebaseCred("shapelessguy@hotmail.it", "180393acer"); FirebaseClass.FireBaseLogIn(); }
            if (token == null) { Console.WriteLine("LogIn richiesto.."); error = true; }
            await Download_IP(Path.Combine(Principal.pathIP, "IP.txt"), "public/IP.txt");
        }

        public static async Task UploadFile_onStorage(string file_local, string file_storage)
        {
        ...
        public static async Task Download_IP(string file_local, string file_storage)
        {
            if (token == null) { Console.WriteLine("LogIn richiesto.."); return; }
            FirebaseStorageOptions op = new FirebaseStorageOptions() { AuthTokenAsyncFactory = () => Task.FromResult(token.FirebaseToken) };
            string download_url = "";
            ...
            using (var client = new WebClient())
            {
                try
                {
                    WebRequest request = WebRequest.Create(download_url);
                    WebResponse response = request.GetResponse();
                    using (Stream dataStream = response.GetResponseStream())
                    {
                        StreamReader reader = new StreamReader(dataStream);
                        string responseFromServer = reader.ReadToEnd();
                        ServerSide.SERVER_IP = responseFromServer.Replace("\n", "");
                        Console.WriteLine(responseFromServer);
                    }
                    response.Close();
                    Console.WriteLine("IP found in Firebase");
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Error in downloading the file " + file_storage + " ... "+ ex.Message);
                    return;
                }
            }
        ...
        public static async Task UploadFile_onStorage(string file_local, string file_storage, bool changecreat_time)
        {
            ...
            {
                Console.WriteLine("Error in uploading: " + e.Message);// try { File.Delete(file_local1); } catch (Exception) { };
                return;
            }
            //try { File.Delete(file_local1); } catch (Exception) { };
            return;
    ...
    public static async Task<bool> UpdateImages_FromStorageToLocal(bool forced, int num_icons_tip, int num_icons_met)
    {
        ...
        foreach (FirebaseMetaData metadata in metadata_tipo_storage)
        {
            ...
            if (corrispondenza) //se il file c'è già in locale
            {
                ...
                if (data_storage > data_local || forced)
                {
